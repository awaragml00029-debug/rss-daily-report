...
> **今日焦点**：
REV-ERBα作为昼夜节律调控因子，揭示其在肿瘤细胞谱系可塑性及治疗靶点中的主导作用（ChIP-Seq, ATAC-Seq, RNA-Seq）。

**主要方向**：
- 探索IL-36/TNFSF15通路在泛发性脓疱型银屑病中的免疫细胞通讯网络。
- 研究单细胞转录组揭示尘螨暴露对小鼠肺微环境的影响。
- 鉴定ZEB1作为JAM-A调控因子，影响胰腺癌相关成纤维细胞对溶瘤重组病毒的敏感性。
- 探究SCA3脊髓转录组失调机制。
- 开发CRISPR-Cas9筛选策略，用于免疫健全宿主中转移相关基因的发现。

**技术亮点**：
- 通过单细胞RNA测序（scRNA-seq）和RNA测序（RNA-seq）实现成纤维细胞向功能性视网膜色素上皮细胞的化学重编程。
- 针对免疫健全宿主，开发了名为"StealTHY"的CRISPR-Cas9筛选技术。

### 🧪 测序类博客

> **今日焦点**：
首次将单细胞RNA测序与质谱联用，实现了同一植物细胞内基因表达与代谢物谱的匹配分析；新型P-body-seq技术实现了对P体内容物的全面剖析。

**主要方向**：
- 单细胞水平揭示基因表达调控植物细胞代谢产物生成机制。
- 探索RNA凝聚体（P体）如何通过储存未翻译RNA调控脊椎动物干细胞命运。

**技术亮点**：
- 单细胞RNA测序与质谱联用分析技术。
- P-body-seq技术实现对RNA凝聚体内容的全面分析。
