...
> **今日焦点**：
NFIL3被发现是调控ILC命运的关键转录因子；一项研究揭示了DNA甲基化谱预测晚期胃癌对PD-1免疫治疗的反应。

**主要方向**：
*   **细胞命运决定与分化**： NFIL3在淋巴样祖细胞中指定ILC命运；Notch2和RBPJ缺陷对单核吞噬细胞及骨髓祖细胞的影响。
*   **疾病机制与治疗**： 广义脓疱型银屑病中，IL-36/TNFSF15信号通路介导的中性粒细胞-角质形成细胞通讯网络；SCA3脊髓中的转录失调；胃癌免疫治疗反应的DNA甲基化预测；小鼠肺微环境对慢性尘螨暴露的单细胞转录组响应。
*   **细胞重编程与再生**： 纤维原细胞化学重编程为功能性视网膜色素上皮细胞；血管内皮细胞重编程促进血管再生。

**技术亮点**：
*   **多组学整合分析**： 结合单细胞RNA测序（scRNA-seq）、RNA测序（RNA-seq）、DNase-seq、ChIP-Seq和Targeted Bisulfite-Seq等高通量测序技术，深入解析细胞功能、基因调控和疾病机制。
*   **CRISPR筛选技术**： StealTHY技术用于免疫活体宿主中的CRISPR-Cas9筛选，以揭示转移的潜在调控因子。

### 📊 R-blogs

> **今日焦点**：
提出一种新型方法，**无需手动编写命令式代码即可构建可复现的数据科学流程**，显著降低了构建复杂数据科学管线的门槛。

**主要方向**：
- 简化可复现数据科学流程的构建过程。
- 探索自动化解决数据科学管线构建问题的途径。

**技术亮点**：
- 引入 {rixpress} 包，提供一种**非命令式**的方式来组织和执行数据科学任务。
- 强调通过**实际案例对比**，展示 {rixpress} 相较于传统命令式编程的优势。

### 🧪 测序类博客

> **今日焦点**：
首次将单细胞RNA测序与质谱联用，实现单株植物细胞内基因表达与代谢物谱的同步分析；新型P-body-seq技术实现对P体内容的全面剖析。

**主要方向**：
- 关联植物细胞基因表达与代谢产物生成机制
- 阐明RNA凝聚体P体在调控脊椎动物干细胞命运中的作用
