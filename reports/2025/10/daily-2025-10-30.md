...
>
> *Powered by [科研普拉斯](https://kyplus.de) & [Claude](https://claude.ai)*
...
> **今日焦点**：
NFIL3被确认为决定ILC命运的关键转录因子，其在淋巴祖细胞中的作用得到 scRNA-seq、RNA-seq及DNase-seq等多维度验证，为ILC研究开辟新途径。

**主要方向**：
*   **细胞命运决定与分化**：NFIL3对ILC谱系决定的核心作用；Notch2与RBPJ在单核吞噬细胞及骨髓祖细胞中的功能比较；成纤维细胞向视网膜色素上皮细胞的化学重编程。
*   **炎症与疾病机制**：广义脓疱型银屑病中，IL-36/TNFSF15信号通路介导的中性粒细胞-角质形成细胞通讯网络；SCA3脊髓转录失调的疾病机制；慢性尘螨暴露对小鼠肺微环境的影响。
*   **肿瘤治疗与预测**：晚期胃癌抗PD-1治疗的DNA甲基化预测模型；ZEB1对胰腺癌相关成纤维细胞对溶瘤重组病毒敏感性的调控；REV-ERBα作为肿瘤谱系可塑性的主调节因子及治疗靶点。

**技术亮点**：
*   **多模态单细胞/高通量测序**：结合scRNA-seq、RNA-seq、DNase-seq等技术，深入揭示NFIL3在ILC命运决定中的作用。
*   **CRISPR-Cas9基因筛选平台**：StealTHY平台支持在免疫活性宿主中进行CRISPR-Cas9筛选，发现新的转移调控因子。

### 📊 R-blogs

> **今日焦点**：
{rixpress} 包提供了一种更便捷的方式来构建可复现的数据科学管道，无需直接编写命令式代码。
...
**技术亮点**：
- 提供 {rixpress} 包，作为命令式代码的替代方案。
- 通过实例演示，说明 {rixpress} 包的优势。

### 🧪 测序类博客

> **今日焦点**：
首次实现单细胞水平基因表达与代谢产物生产的匹配，以及新型P-body测序技术（P-body-seq）用于全面解析RNA凝聚体内容。

**主要方向**：
-  解析单株植物细胞内基因表达与代谢产物生产的关联。
-  揭示P-body在脊椎动物干细胞命运决定中的作用机制。

**技术亮点**：
-  结合单细胞RNA测序与质谱技术，实现单细胞基因表达与代谢组学的同步分析。
-  开发P-body-seq，实现对P-body内RNA内容的全面剖析。
