...
> **今日焦点**：
NFIL3转录因子在淋巴祖细胞分化为ILC（固有淋巴细胞）中的决定性作用被揭示，为ILC研究提供了关键分子机制。

**主要方向**：
*   **细胞命运决定与分化**：NFIL3在ILC分化中的必要性和充分性；Notch2和RBPJ信号通路对单核吞噬细胞和髓系祖细胞的影响。
*   **疾病机制与治疗靶点**：IL-36/TNFSF15信号通路在泛发性脓疱型银屑病中的炎症反应网络；REV-ERBα作为肿瘤谱系可塑性的调控因子及治疗靶点；SCA3脊髓中的转录失调机制。
*   **组织再生与修复**：化学重编程成视网膜色素上皮细胞；血管化再生与内皮细胞重编程。

**技术亮点**：
*   **多组学整合分析**：结合单细胞RNA测序（scRNA-seq）、RNA测序（RNA-seq）和DNase-seq等技术，深入解析细胞命运决定和疾病机制。
*   **新型CRISPR筛选平台**：StealTHY平台实现免疫活性宿主中的CRISPR-Cas9筛选，以揭示转移性调控因子。

### 📊 R-blogs

> **今日焦点**：
通过对比使用 {rixpress} 包与不使用该包构建可复现数据科学管线，揭示了 {rixpress} 在简化和自动化这一过程中的关键价值。

**主要方向**：
*   构建可复现的数据科学管线。
*   评估和展示新型工具 ({rixpress}) 在简化复杂任务中的作用。

**技术亮点**：
*   通过实际案例对比，直观展示 {rixpress} 相比传统命令式编程在构建可复现管线上的优势，可能涉及自动化、简化流程等方面。

### 🧪 测序类博客

> **今日焦点**：
首次将单细胞RNA测序与质谱联用，实现同一植物细胞内基因表达与代谢产物谱的同步分析，为理解细胞功能调控提供新视角。

**主要方向**：
- 植物单细胞基因表达与代谢产物关联研究
- RNA凝聚体（P-body）调控发育决策的机制解析

**技术亮点**：
- P-body-seq：一种新型RNA测序技术，可全面分析P-body内容，揭示其在干细胞命运决定中的作用。
- 单细胞多组学联用：整合基因表达与代谢组学信息，实现对单细胞分子活动的深度洞察。
