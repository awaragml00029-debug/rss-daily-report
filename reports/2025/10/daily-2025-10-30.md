...
> **今日焦点**：
NFIL3被确认为指定固有淋巴细胞（ILC）谱系的关键转录因子，其在淋巴祖细胞中的作用首次被明确。

**主要方向**：
- 揭示NFIL3在ILC分化命运中的必要性和充分性。
- 探索NFIL3在ILC2激活和过敏性肺部炎症中的作用。
- 分析NFIL3在免疫细胞发育中的转录调控机制。

**技术亮点**：
- 结合单细胞RNA测序（scRNA-seq）和RNA测序（RNA-seq）等高通量测序技术，以及DNase-seq等基因组结合分析，深入解析了NFIL3的功能。

### 📊 R-blogs

> **今日焦点**：
文章探讨了在构建可复现数据科学流水线时，如何通过 {rixpress} 包简化命令式代码编写，实现更高效的流水线构建。

**主要方向**：
- 简化和自动化数据科学流水线中的命令式代码编写。
- 探索 {rixpress} 包在解决流水线构建问题上的优势。

**技术亮点**：
- 提出 {rixpress} 包作为一种新型工具，旨在降低构建可复现数据科学流水线的门槛。
- 通过对比未使用 {rixpress} 包的解决方案，突显其在提升效率和简化流程方面的创新性。

### 🧪 测序类博客

> **今日焦点**：
首次实现单细胞层面基因表达与代谢产物生产的精准匹配，揭示RNA凝聚体P-body在调控干细胞命运中的关键作用。

**主要方向**：
-   解析植物单细胞内基因表达调控代谢产物生成的机制。
-   阐明P-body如何通过储存未翻译RNA来指导脊椎动物的干细胞发育决策。

**技术亮点**：
-   创新性地将单细胞RNA测序与质谱联用，实现单细胞基因表达和代谢物谱的同步分析。
-   开发P-body-seq技术，实现对P-body内RNA内容的全面剖析。
