...
> **今日焦点**：
NFIL3转录因子是决定ILC命运的关键，其在淋巴祖细胞分化中起必要且充分的作用。

**主要方向**：
- 探索NFIL3在淋巴细胞发育中的作用机制。
- 揭示NFIL3在特定细胞类型（如ILC2）激活和过敏性肺部炎症中的调控。
- 研究NFIL3在免疫细胞（单核吞噬细胞、髓系祖细胞）发育中的功能。

**技术亮点**：
- 综合运用单细胞RNA测序、RNA测序（多种特异性分析）、DNase-seq等高通量测序技术，以及CRISPR-Cas9筛选技术，深入解析基因功能和细胞命运决定机制。

### 📊 R-blogs

> **今日焦点**：
引入新型R包{rixpress}，旨在简化可复现数据科学流水线构建，并展示无需该工具即可解决相同问题的对比方法。

**主要方向**：
- 探索构建可复现数据科学流水线的新型工具。
- 对比使用和不使用{rixpress}包解决数据科学问题的效果。

**技术亮点**：
- 提出{rixpress}包，作为一种更便捷的方式来构建可复现的数据科学流水线。
- 通过对比实验，验证{rixpress}在简化复杂流程方面的优势。

### 🧪 测序类博客

> **今日焦点**：
首次实现单植物细胞内基因表达与代谢物生产的同步分析，揭示RNA调控干细胞命运的新机制。

**主要方向**：
- 单细胞水平解析植物基因表达与代谢物生产的关联。
- 探索P-body在脊椎动物干细胞命运决定中的作用。

**技术亮点**：
- 结合单细胞RNA测序与质谱联用技术。
- 开发P-body-seq技术实现对P-body内RNA内容的全面分析。
