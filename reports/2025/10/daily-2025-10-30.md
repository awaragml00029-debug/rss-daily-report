...
> **今日焦点**：
NFIL3被确认为指定淋巴源性祖细胞ILC谱系的必需且充分转录因子，为理解ILC发育提供了关键机制。

**主要方向**：
*   **细胞通讯与疾病机制**：揭示了脓疱型银屑病中IL-36/TNFSF15通路介导的中性粒细胞-角质形成细胞通讯网络，以及SCA3脊髓中的转录失调。
*   **谱系决定与细胞重编程**：阐明了NFIL3对ILC命运的决定作用，并展示了化学重编程将成纤维细胞转化为功能性视网膜色素上皮细胞。
*   **肿瘤免疫与治疗靶点**：探索了晚期胃癌对抗PD-1治疗的DNA甲基化预测标志物，以及REV-ERBα作为肿瘤谱系可塑性的调控因子和治疗靶点。

**技术亮点**：
*   **单细胞与高通量测序**：广泛应用于比较不同条件下细胞亚群的转录组差异，如Notch2/RBPJ缺陷细胞、小鼠肺部微环境、以及ILC2激活机制。
*   **基因组调控分析**：利用DNase-seq和ChIP-seq技术，解析了NFIL3在基因组上的结合位点以及REV-ERBα的转录调控网络。

### 📊 R-blogs

> **今日焦点**：

通过一个无需 {rixpress} 包即可构建可复现数据科学管道的实例，直观展示了该包的价值和必要性，强调了自动化构建数据科学工作流的潜力。
...
*   通过解决具体问题来论证 {rixpress} 包的优势。
*   提供了一种无需直接编写命令式代码即可实现数据科学流程的方法。

### 🧪 测序类博客

> **今日焦点**：
首次将单细胞RNA测序与质谱联用，实现单株植物细胞基因表达与代谢物谱的同步分析，揭示了细胞层面的基因-代谢关联。

**主要方向**：
- 植物单细胞基因表达与代谢物生产的关联研究
- RNA凝聚体（P-body）在调控干细胞命运中的作用机制

**技术亮点**：
- P-body-seq：一种新型RNA测序技术，可全面分析P-body内RNA内容。
- 单细胞多组学联用：整合单细胞RNA测序与质谱技术。
