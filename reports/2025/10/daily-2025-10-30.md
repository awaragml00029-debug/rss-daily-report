...
> **今日焦点**：
NFIL3被确认为决定ILC命运的关键转录因子，其作用通过多种高通量测序技术（scRNA-seq, RNA-seq, DNase-seq）得到验证。

**主要方向**：
*   **细胞命运决定与分化**：研究NFIL3在淋巴样祖细胞中决定ILC命运的关键作用。
*   **疾病机制与治疗靶点**：探索IL-36/TNFSF15通路在银屑病中的作用；鉴定REV-ERBα作为肿瘤细胞可塑性的调控因子及治疗靶点；分析SCA3脊髓转录失调的致病机制。
*   **免疫细胞通讯与功能**：解析Notch2和RBPJ缺陷对单核吞噬细胞及骨髓祖细胞的影响；揭示巨噬细胞在小鼠肺微环境中的作用。

**技术亮点**：
*   **多模态高通量测序**：结合单细胞RNA测序（scRNA-seq）、RNA测序、DNase-seq等技术，全面解析基因调控和细胞命运。
*   **CRISPR-Cas9筛选技术**：在免疫活性宿主中应用CRISPR-Cas9技术，以揭示潜在的转移性调控因子。

### 📊 R-blogs

> **今日焦点**：
{rixpress} 包旨在简化可复现数据科学管道的构建，通过提供一种无需直接编写命令式代码的解决方案，降低了用户门槛。
...
**技术亮点**：
*   提供一种新型的、更易于理解和实现的方式来构建可复现的数据科学管道。

### 🧪 测序类博客

> **今日焦点**：
首次实现单植物细胞内基因表达与代谢物产量的匹配分析；新型P-body-seq技术实现对P体内容的全面分析。

**主要方向**：
- 揭示单细胞水平基因表达调控代谢物产生机制。
- 阐明P体在调控脊椎动物干细胞命运中的作用。

**技术亮点**：
- 整合单细胞RNA测序与质谱技术。
- 开发P-body-seq实现对P体内容物的全面剖析。
