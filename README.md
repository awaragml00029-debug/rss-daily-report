...
**1. 筛选时间优化为UTC-5 & Hugo周数标签支持** (2025-11-04)
- 🕐 筛选时间从UTC改为UTC-5（美国东部时间）
  - 更符合实际使用场景
  - 数据筛选基准时间调整为 `datetime.utcnow() - timedelta(hours=5)`
  - 显示时间仍使用北京时间（UTC+8）
- 🏷️ Hugo Front Matter自动添加周数标签
  - 从Google Sheet B列（attribute字段）自动提取周数
  - 添加到Front Matter的tags中（如：`Week1`, `Week2`）
  - 便于在Hugo博客中按周筛选和分类报告
  - 示例：
    ```yaml
    tags:
...
**3. 关键词配置优化** (2025-11-04)
- 🎯 新增免疫相关：免疫、免疫微环境、TME
- 🧬 新增心血管/神经系统：心血管、心脏、脑瘤、阿尔兹海默
- 💊 新增激酶/耐药相关关键词
- 📊 新增数据库资源：TCGA、DepMap
- 🌐 新增英文排除关键词：advertisement、recruitment、correction、retraction等
- 📖 保留泛化词汇（肿瘤、免疫、代谢）实现更广泛的内容覆盖

**4. 优化 HTML 链接行为** (2025-11-03)
- 🔗 外部链接（查看原文）在新窗口打开
- 📍 内部锚点链接（详见 文末）在本页跳转
- 🔒 添加 `rel="noopener noreferrer"` 提升安全性

**5. 简化工作流架构** (2025-11-03)
...
- **数据筛选时区**：UTC-5（美国东部时间）
  - 使用 `datetime.utcnow() - timedelta(hours=5)` 作为筛选基准
  - 符合实际数据采集时区需求
- **显示时区**：北京时间（UTC+8）
  - 报告中的日期和时间显示为北京时间
  - 文件命名使用北京时间日期
- **周数标签**：自动从Google Sheet B列提取
  - 在Hugo Front Matter中自动添加到tags
  - 便于按周筛选和分类
